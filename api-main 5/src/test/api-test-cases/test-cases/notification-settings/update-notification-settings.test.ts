import { prismaMock } from '@core/prisma/singleton';
import request from 'supertest';
import { testApplication } from '../../test-application';
import * as Constants from '../../core/constants';
import { RequestMethod } from '../../core/enums';
import { verifyEndpointFailsWithoutAuthenticationTestCase } from '../../core/common';
import { mockAuthentication } from '@test/api-test-cases/core/authentication';
import { generateUser } from '@test/utils/generate';
import userRepository from '@repositories/user.repository';
import { userService } from '@modules/index/index.service';

const userObject = generateUser({ id: 2 });

beforeEach(() => {
  jest.spyOn(userRepository, 'findFirst').mockResolvedValue(userObject);
  jest.spyOn(userService, 'findByUserGuid').mockResolvedValue(userObject);
});

describe('API Test Suite: Update Notification Settings Endpoint', () => {
  mockAuthentication();

  verifyEndpointFailsWithoutAuthenticationTestCase(
    Constants.NOTIFICATION_SETTINGS_ENDPOINT,
    RequestMethod.PUT
  );

  // TODO: Cannot create valid test case.
  // In notification-settings.service.ts, prisma.$transaction is ued which generates
  // an error: Please make sure your database server is running at `test`:`5432`.
  //
  // We need to determine how to mock a databack server connection.
  //
  // For now, this test case will be skipped

  test.skip('should be successful with valid parameters', (done) => {
    //Setup
    prismaMock.notificationSettings.findMany.mockResolvedValue([
      Constants.SAMPLE_NOTIFICATION_SETTINGS_DATA,
    ]);
    prismaMock.notificationSettings.update.mockResolvedValue(
      Constants.SAMPLE_NOTIFICATION_SETTINGS_DATA
    );

    //When
    request(testApplication)
      .put(`${Constants.NOTIFICATION_SETTINGS_ENDPOINT}`)
      .set(Constants.AUTHORIZATION_REQUEST_KEY, Constants.VALID_TOKEN)
      .send([
        {
          type: 'deviceIssues',
          option: 'immediately',
          sms: true,
          push: true,
        },
      ])
      //Then
      //   .expect(204, done);
      .end((error, response) => {
        if (error) {
          return done(error);
        }
        return done();
      });
  });
});
