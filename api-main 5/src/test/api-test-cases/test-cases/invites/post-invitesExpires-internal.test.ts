...
describe('API Test Suite: Expire Invite Endpoint', () => {
  const skip = 0;
  const take = 10;
  const periodDays = 2;
  ...
  test('should be successful with valid parameters', (done) => {
    ...
    request(testApplication)
      .post(`/internal/invites/expires/${periodDays}?skip=${skip}&take=${take}`)
      //Then
      .end((error, response) => {
        if (error) {
          return done(error);
  ...
  test('should fail if skip is string', (done) => {
    request(testApplication)
      .post(
        `/internal/invites/expires/${periodDays}?skip=${'skip'}&take=${take}`
      )
      .end((error, response) => {
        if (error) {
          return done(error);
